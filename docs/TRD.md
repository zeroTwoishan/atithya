...
| LLM | **Any of Claude / Gemini / Groq**, picked from whichever API key is set (`src/agents/llm.js`) | Tool-use / structured output for listing extraction and itinerary generation. The deck names no LLM vendor, and Gemini and Groq both have a free tier needing no card — on a student team that is the difference between the agent speaking Hindi on stage and not. Every agent calls `extract()`/`say()` and gets `null` on failure, so no agent knows or cares which one is live |
