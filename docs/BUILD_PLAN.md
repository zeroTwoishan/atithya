...
|---|---|---|
| **R1 — Backend/Agent Lead** | Express app + `schema.sql` init, LangGraph.js supervisor, Planning & Booking Agent | `TRD` §3.2, §4 (API surface) |
| **R2 — Onboarding Agent Engineer** | WhatsApp Cloud API integration, Onboarding Agent graph, Verification Agent | `TRD` §3.1, §3.3 |
...
| **R5 — Data Engineer** | `schema.sql` applied, seed data (§5 of [[BACKEND_SCHEMA]]), pgvector embeddings, gov aggregate queries | `src/db/schema.sql`, `npm run db:seed` |
...
- R1: `npm init` + Express 5, `cors`/`jsonwebtoken`/`pg`/`zod` installed, one router per domain (`listings`, `trips`, `analytics`, `whatsapp`), hosted/local Postgres up with `pgvector` extension enabled.
- R2: create a Meta app, add the WhatsApp product, grab the test number + permanent token, register the webhook callback URL and verify token (use `ngrok`/similar for local dev), add each demo phone as a recipient (test numbers cap at 5).
- R3 + R4: `npm create vite@latest` (React, JS), Tailwind v4 installed with the two `@theme` token sets from [[UI_UX_DESIGN]] §2/§3, `@phosphor-icons/react`/`react-router-dom`/`@tanstack/react-query`/`axios` installed. Pull the 12 Stitch-generated HTML screens from `docs/screens/` as the visual/content reference (§4b of [[UI_UX_DESIGN]] — not copy-pasted, translated to components), wire static routing (no live data yet).
- R5: apply `schema.sql` (`npm run db:init`) and write the query helpers per domain — this can happen mostly offline, doesn't block on anyone else.
- R6: confirm every team member has API keys (Anthropic, Meta app secret + access token, DB URL) in `.env`, not committed. **R1 + R6 also freeze the exact JSON request/response shape for every [[TRD]] §4 endpoint right now**, before R3/R4 write a line of API-calling code — AMS-with-TimeTable's own history is full of post-hoc "fix: read the field from X not Y" commits from exactly this kind of skipped step. A shared shape doc (even a 1-page markdown table) beats finding the mismatch at hour 10.
- **Checkpoint:** DB reachable by everyone, one person can send a WhatsApp sandbox message and see the webhook fire (even if it just logs to console), Vite dev server + Express server both running with CORS working between them, API contract frozen and shared.

### Hours 2–8: Sprint 1 — one thing working end-to-end, not everything half-working
- R1: `/api/v1/listings` router (CRUD), `/api/v1/trips` create + basic itinerary compose (can start with known_sites only, no pgvector yet).
...
1. Government dashboard beyond the heatmap placeholder — the deck itself marks the district heatmap "PLANNED, weeks 9-12", so a stylized placeholder is the honest state, not a gap.
...
**Never cut:** the onboarding-agent-to-tourist-booking loop, and none of the 4 supervisor routes — the deck sells "4 agents under one LangGraph supervisor", so re-planning is baseline now, not the first thing to drop — that's the entire pitch. If time runs out, everything else is negotiable before that isn't.
